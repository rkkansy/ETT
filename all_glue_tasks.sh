...
learning_rates=("4e-5")
batch_sizes=(16)
task_names=("mrpc" "cola" "rte" "sst2" "stsb")
long_task_names=("qqp" "qnli" "mnli")
# Path to the Python script
python_script=run_glue_n.py
model_path="/home/robert/Documents/ETT/models/checkpoint-00019712-amb-24h"
model_name="24h_amb_no-dp_"
# Common parameters
...
for lr in "${learning_rates[@]}"; do
    for batch_size in "${batch_sizes[@]}"; do
        for task_name in "${task_names[@]}"; do
            ...
            python $python_script $common_params --eval_steps 100 --task_name $task_name --learning_rate $lr --per_device_train_batch_size $batch_size --per_device_eval_batch_size $batch_size --output_dir $output_dir 
        done
        for task_name in "${long_task_names[@]}"; do
            ...
            # Execute the Python script with the current parameters
            python $python_script $common_params --eval_steps 10000 --task_name $task_name --learning_rate $lr --per_device_train_batch_size $batch_size --per_device_eval_batch_size $batch_size --output_dir $output_dir 
        done
    done
done
